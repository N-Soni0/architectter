...
export const useShapeStore = create(
	immer<State & Actions>((set) => ({
		...
		addTile: (newTile) => {
			set((store) => {
				store.shape.tiles.push(newTile);
			});
		},

		removeTile: (removeTileID) => {
			set((store) => {
				store.shape.tiles = store.shape.tiles.filter(
					(tile) => getTileId(tile) !== getTileId(removeTileID)
				);
			});
		},
		...
		moveTile: (movedTile, newCoordinates) => {
			set((store) => {
				const tile = store.shape.tiles.find(
					(tile) => getTileId(tile) === getTileId(movedTile)
				);
